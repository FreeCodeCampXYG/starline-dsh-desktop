...
Windows 主程序 manifest 已声明 `requireAdministrator`，因此启动时会由 UAC 请求管理员令牌，并让宿主直接创建的 Node/DSH 子进程继承同一令牌。该设置用于需要提升令牌的本地操作，不会移动或重建 DSH Profile，也不会自动关闭 DSH 自身的沙箱权限。

管理员令牌不能绕过火绒的信誉、行为或勒索防护策略。若仍被拦截，请先确认下载资产的 SHA-256，再在火绒中分别检查 Desktop、相邻 `offline-runtime\\node.exe` 和 DSH 入口的拦截记录；不要直接关闭全局防护。分享日志时删除 Token、Cookie、API Key、用户名和私人路径。

当前自动化证据覆盖本地 NTFS 中文和空格路径；UNC 网络共享、映射盘断连和超长路径尚未作为支持目标验证。

### 卸载与重装

- Windows Setup：从开始菜单的“卸载 Starline DSH Desktop”、Windows“已安装的应用”，或安装目录的 `uninstall.exe` 卸载；新版 Setup 会沿用注册表中的安装目录。
- Windows/macOS/Linux 便携包：先退出应用，再删除完整解压目录或 macOS 应用包；不要只删除 EXE 或正在运行的 `offline-runtime` 文件。
- Ubuntu DEB：使用 `sudo apt remove starline-dsh-desktop`；apt 会移除系统程序和菜单项。
- 用户配置、日志和 DSH 工作区默认保留；确认不再需要时再单独清理用户目录。
...
```bash
npx --yes --package=@deepseek-ai/dsh@0.1.1-rc.2 dsh web --no-open
```
...
1. 启动后的自动检查先直连官方 npm registry，失败回退国内镜像并显示 `latest` 与 `next`；手动刷新和应用版本按自定义代理、应用启动时继承的系统环境代理、国内镜像直连依次访问受信 registry。检查失败不会阻塞当前 DSH 启动。
...
4. `offline-full` 默认仍使用包内固定版本；如果系统已安装 Node.js 22.19+ 或 24+，应用确认更新后会切换到系统 Node/npm 在线下载精确版本，不会修改包内 `node_modules`，更新失败会回退到离线版本。若系统没有 Node/npm，更新按钮会直接说明原因，此时请下载包含该 DSH 版本的新离线资产。离线资产是便携归档，应关闭旧程序后解压到新目录验证；手工覆盖运行中的 `node_modules` 会破坏完整性与原生依赖门禁。
5. 进度条显示真实启动阶段而非 npm 下载字节。停在 65% 表示 npx 子进程已启动、仍在解析或准备依赖；默认最多等待 5 分钟（可在设置中改为 30–600 秒），查看 Starline 日志中的 registry、超时和 npm 调试日志判断是否仍有缓存命中或明确错误。
