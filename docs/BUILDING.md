...
- npm
- Wails CLI 2.14.0
- 平台原生 C/C++ 工具链与 WebView 开发包；Windows 托盘依赖仅随 Windows 构建编译，Linux 不需要额外托盘开发包
...
```powershell
.\scripts\prepare-offline-runtime.ps1 -DSHVersion 0.1.2-alpha.3
```

```bash
bash scripts/prepare-offline-runtime.sh 0.1.2-alpha.3
```
...
GitHub Actions 的 Go 模块/编译缓存和 npm 下载缓存由 `actions/setup-go`、`actions/setup-node` 管理；Wails CLI 二进制按操作系统和 runner 架构复用，Windows Release 额外缓存固定版本的 NSIS 安装目录。缓存只用于构建加速，不进入最终安装包或离线运行时；缓存恢复后仍会执行 `makensis` 校验、离线运行时完整性校验和平台功能测试。NSIS 首次缓存未命中时下载固定的 `nsis.install 3.11.0` 包、核对仓库记录的 SHA-256，再从 runner 临时目录安装，不依赖 Chocolatey OData 搜索结果。
