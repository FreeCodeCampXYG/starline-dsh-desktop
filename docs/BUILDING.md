...
```powershell
.\scripts\build-windows.ps1 -Version 0.2.5
```

额外构建 Windows x64 离线便携包：

```powershell
.\scripts\build-windows.ps1 -Version 0.2.5 -OfflineFull
```
...
```powershell
wails build -clean -trimpath -platform windows/amd64 -nsis -installscope user -ldflags "-s -w -H=windowsgui -X main.version=0.2.5"
```
...
```bash
npm --prefix frontend ci
wails build -clean -trimpath -platform darwin/arm64 -ldflags "-s -w -X main.version=0.2.5"
```
...
```bash
npm --prefix frontend ci
wails build -clean -trimpath -platform linux/amd64 -tags webkit2_41 -ldflags "-s -w -X main.version=0.2.5"
```
