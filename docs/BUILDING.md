...
- npm
- Wails CLI 2.14.0
- 平台原生 C/C++ 工具链、WebView 开发包与系统托盘开发包（Linux 需要 `libayatana-appindicator3-dev`）
...
```powershell
.\scripts\build-windows.ps1 -Version 0.3.0 -OfflineFull
```
...
```powershell
wails build -clean -trimpath -platform windows/amd64 -nsis -installscope user -ldflags "-s -w -H=windowsgui -X main.version=0.3.0"
```
...
```bash
npm --prefix frontend ci
wails build -clean -trimpath -platform darwin/arm64 -ldflags "-s -w -X main.version=0.3.0"
```
...
```bash
npm --prefix frontend ci
wails build -clean -trimpath -platform linux/amd64 -tags webkit2_41 -ldflags "-s -w -X main.version=0.3.0"
```
