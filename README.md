...
- DSH Market 沿用同一子进程网络，并固定现有 Profile 已记录的 pnpm Store，避免全局 `store-dir` 被其他项目修改后产生 Store 冲突；
...
- 同一登录会话只运行一个宿主机进程：第二次启动会把已有窗口带回前台后退出，不会启动第二份 DSH。多个宿主共用同一份 DSH 用户数据目录时必然争用会话写锁（前端显示 `SessionAlreadyOwnedError`），因此多开被显式收敛为单实例；配置保存仍带跨进程原子锁和过期版本检查，避免手工运行的 DSH 或其他构建静默覆盖代理设置；
...
宿主会显式传入 `--no-open`：官方 DSH 默认会把 Web 地址交给系统浏览器，但桌面端会为当前进程建立仅监听 loopback 的 Go 代理。代理在内存中完成一次 token→cookie 握手，iframe 和“在浏览器中打开”都访问不含 token 的代理地址，因此不会依赖 WebView 的第三方 cookie；代理随 DSH 进程退出而关闭。
...
| 网络 | 普通包默认优先官方 npm registry，失败回退国内镜像；`offline-full` 启动不访问 npm，但模型服务等功能可能仍需网络 |
...
当前 main 默认启动 `@deepseek-ai/dsh@0.1.5-rc.1`，下一轮 `offline-full` 也固定 0.1.5-rc.1。桌面端启动后的自动版本检查固定优先直连官方 npm registry，失败回退国内镜像，顶栏只提示 npm `latest` 与 `next`，不会静默切换；手动刷新和应用版本依次尝试自定义代理、应用启动时继承的 HTTP(S) 环境代理和国内镜像直连。普通包准备先探测官方 npm registry，失败再回退国内镜像；它只解决 Node/npm/DSH 准备，DSH 模型/API 请求如需代理仍应恢复可用代理并重启。在线包由用户确认通道后保存精确版本、回收当前 DSH 子进程树并重启；普通包的在线启动页面校验默认最多等待 5 分钟，可在“代理与启动设置”中调整为 30–600 秒，新版本不完整时会自动恢复更新前的版本和配置。离线包默认继续使用包内固定版本；如果电脑另外安装了 Node.js 22.19+ 或 24+，确认更新后会改用系统 Node/npm 在线下载新版本，不改写离线依赖闭包，仍可通过“恢复默认”回到包内版本；没有系统 Node/npm 时需下载包含新版本的离线包。`DSH_DESKTOP_DSH_VERSION` 可用于临时开发覆盖，并优先于界面设置。
...
4. 原生窗口先短暂隐藏预热；宿主确认本地 DSH 已监听后，启动仅监听 loopback 的内存认证代理，并让内嵌 WebView 的 iframe 加载不含 token 的代理 URL；如果代理或 WebView 仍异常，可从桌面工具手动在系统浏览器中打开同一代理入口；
...
普通包不会直接调用 PATH 中版本未知的全局 `dsh`，而是要求桌面端选定的精确版本（当前 main 默认 `@deepseek-ai/dsh@0.1.5-rc.1`，或用户手动确认的 npm 官方通道版本）。npx 会复用与该版本对应的 npm 缓存，因此缓存完整时不会重复下载包体；但 npm 仍可能校验依赖元数据，首次准备或缓存不完整时耗时较长。启动自动检查直连官方 npm registry，失败回退国内镜像；手动刷新和应用版本按自定义代理、系统环境代理、国内镜像直连依次尝试。
...
应用会在启动后先检查官方 npm registry，失败再回退国内镜像并告知 npm `latest` 与 `next`；手动刷新或应用版本按自定义代理、系统环境代理、国内镜像直连依次尝试。为避免预发布版本和原生依赖在后台静默漂移，切换通道仍需用户确认；应用后保存的是精确版本。DSH 模型/API 请求仍可能需要可用代理。新版本下载不完整或 DSH 本地页面校验失败时，应用会恢复更新前的版本和配置并重启旧运行时。`offline-full` 在确认新版本后只切换到系统 Node/npm 在线运行时，不会原地修改包内 `node_modules`；电脑没有系统 Node/npm 时仍需下载包含新版本的离线资产。当前 main 的下一轮 `offline-full` 已固定 0.1.5-rc.1，但仍必须通过 GitHub Actions 六平台原生依赖与最终归档门禁才能成为发布资产。Desktop 自身通过“桌面工具 → 检查 Desktop 更新”打开本项目最新 Release，由用户选择对应平台资产，不会静默覆盖安装。
