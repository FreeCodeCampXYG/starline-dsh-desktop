...
try {
    ...
    wails build -clean -trimpath -platform windows/amd64 -nsis -installscope user -ldflags "-s -w -H=windowsgui -X main.version=$Version"
    ...
    Compress-Archive -LiteralPath $binaryPath -DestinationPath $portablePath -Force
    ...
    foreach ($artifact in @($portablePath, $setupPath)) {
    ...
    Get-Item -LiteralPath $portablePath, $setupPath | Select-Object Name, Length, LastWriteTime
}
finally {
